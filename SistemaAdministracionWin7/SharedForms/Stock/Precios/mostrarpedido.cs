...
namespace SharedForms.Stock
{
    ...
    {
        pedidoData _pedido;
        public mostrarpedido(pedidoData p)
        {
            _pedido = p;
            InitializeComponent();
        }
        ...
        private void calcularSubTotal()
        {
			...
			{
                subt += HelperService.ConvertToDecimalSeguro(tabla[7, i].Value);
			}
        ...
        private void AgregarArticulo()
        {
            ...
            if (HelperService.esCliente(GrupoCliente.Opiparo))
            {
                ...
                tabla[6, fila].Value = 1;
                tabla[7, fila].Value = txtprecio.Text;
        ...
        {
             pedidoService = new PedidoService(new PedidoRepository(), new PedidoDetalleRepository());
            ...
            if (_pedido!=null)
            {
                _pedido = pedidoService.GetByID(_pedido.ID);
                cargarpedido(_pedido);
            }
            
        }

        private void cargarpedido(pedidoData _pedido)
        {
            cmbClientes.SelectedItem = _pedido.Cliente;
            cmbVendedor.SelectedItem = _pedido.Vendedor;
            lblnumerofactura.Text = _pedido.NumeroCompleto;
            lblNumInicio.Text = _pedido.Prefix.ToString();
            checkCompleto.Checked = _pedido.completo;
            var colorService = new ColorService(new ColorRepository());
            int fila;
            var productoService = new ProductoService(new ProductoRepository());
            foreach (pedidoDetalleData item in _pedido.Children)
            {
                tabla.Rows.Add();
                ...
                tabla[1, fila].Value = productoService.GetProductoByCodigoInterno(item.codigo,false).First().Show;
                tabla[2, fila].Value = colorService.GetByCodigo(item.codigo).Description;

                tabla[3, fila].Value = item.codigo.Substring(10,2);
                tabla[5, fila].Value = item.precio;
                tabla[6, fila].Value = item.cantidad;
                tabla[7, fila].Value = item.precio*item.cantidad;

            }


            txtdescuento.Text = _pedido.Descuento.ToString();
            txtsubtotal.Text = _pedido.Monto.ToString();
        ...
        private void obtenerNumeroPedido()
        {
            ...
            lblNumInicio.Text = HelperService.Prefix.ToString();
            lblnumerofactura.Text =
                (pedidoService.GetNextNumberAvailable(HelperService.IDLocal, HelperService.Prefix, false));
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (dg == DialogResult.OK)
            {
                ...
                pedidoService.marcarComo(_pedido.ID, checkCompleto.Checked);
            MessageBox.Show("Pedido Actualizado ", "Alerta", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void procesarVenta()
        {
            ...
            {
                pedidoDetalleData detalle = new pedidoDetalleData();

                detalle = new pedidoDetalleData();
                detalle.FatherID = ventaID;
                detalle.precio = HelperService.ConvertToDecimalSeguro(tabla[5, fila].Value) ;
                detalle.cantidad = !(Convert.ToBoolean(tabla[4, fila].Value)) ? Convert.ToInt32(tabla[6, fila].Value.ToString()) : -1 * Convert.ToInt32(tabla[6, fila].Value.ToString());
                detalle.codigo = tabla[0, fila].Value.ToString();
                nuevaVenta.Children.Add(detalle);

               
            }
            pedidoService.Insert(nuevaVenta);
            ...
            if (stock<0)
	{
        ...
        private void calcularCantidad(int rowIndex)
        {
            ...
            tabla[7, rowIndex].Value = HelperService.ConvertToDecimalSeguro(tabla[5, rowIndex].Value.ToString()) * Convert.ToInt32(tabla[6, rowIndex].Value.ToString());
            calcularSubTotal();
        }
