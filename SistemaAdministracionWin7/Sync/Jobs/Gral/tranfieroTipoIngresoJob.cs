...
namespace Sync.Jobs
{
    class tranfieroTipoIngresoJob : LoggeableJob, IJob
    {
        public tranfieroTipoIngresoJob()
            : base("tranfieroTipoIngresoJob")
        {
        }
        ...
        public static IJobDetail Get(bool descargar = false, int CountEjecuccionDiferencia = 99999)
        {
            ...
            return JobBuilder.Create<tranfieroTipoIngresoJob>()
                .WithIdentity("tipoIngreso_" + category)
        ...
        private void update(bool download)
        {
            ...
            List<tipoRetiroData> retiroremoto = tipoIngreso.getAll(false, connLocal);
            ...
            List<tipoRetiroData> retirolocal = tipoIngreso.getAll(false, connLocal);
            if (retirolocal != null)
            ...
            foreach (tipoRetiroData p in retiroremoto)
            {
                ...
                {//nuevo
                    aux = BusinessComponents.tipoIngreso.insert(p, connLocal);
                    Log("Insertando " + p.id.ToString() + " en local:" + connLocal.ToString() + "; La operacion resulto: " + aux.ToString());
                }
                else
                {//update
                    aux = BusinessComponents.tipoIngreso.update(p, connLocal);
