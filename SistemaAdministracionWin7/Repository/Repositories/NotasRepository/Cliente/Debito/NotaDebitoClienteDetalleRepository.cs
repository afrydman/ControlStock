...
{
    public class NotaDebitoClienteDetalleRepository : DbRepository, INotaDetalleRepository
    {
        public NotaDebitoClienteDetalleRepository(bool local = true)
            : base(local)
        {
        }
        ...
        public bool InsertDetalle(NotaDetalleData ordenPagoDetalle)
        {
            ...
            {
                var sqlEnvio = @"INSERT INTO [dbo].[notaDebitoClientes_detalles]
           ([FatherID]
           ,[Description]
           ,[Cantidad]
     ...
     VALUES
        ...
        public List<NotaDetalleData> GetDetalles(Guid idnota)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                var sql = "select * from notaDebitoClientes_detalles where FatherID = @FatherID";
