...
{
    public class NotaCreditoClienteRepository: DbRepository,INotaRepository
    {
        public NotaCreditoClienteRepository(bool local=true) : base(local)
        {
        }
        ...
        public bool Insert(NotaData theObject)
        {
            ...
            {
                var sqlEnvio = @"INSERT INTO [dbo].[notacreditoClientes]
           ([id]
           ,[idlocal]
           ,[Numero]
           ,[Prefix]
     ...
     VALUES
        ...
        public bool Disable(Guid idObject)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                var sql = @"	update notacreditoclientes set Enable = '0' where id  = @id;
                        SELECT @@ROWCOUNT;";
        ...
        public List<NotaData> GetAll()
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                ...
                resultado2.ForEach(x => x.tipo = tipoNota.CreditoCliente);
        ...
        public NotaData GetByID(Guid idObject)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                ...
                aux.tipo=tipoNota.CreditoCliente;
                return aux;
            }
        }
        ...
        public NotaData GetLast(Guid idLocal, int Prefix)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                var sql = @"select top 1 " + DEFAULT_SELECT + @"   where nota.Prefix=@Prefix and nota.idlocal=@idlocal  and nota.Numero = (
                            select max(Numero) as 'Numero' from notaCreditoClientes where Prefix=@Prefix and idlocal=@idLocal ) ";
