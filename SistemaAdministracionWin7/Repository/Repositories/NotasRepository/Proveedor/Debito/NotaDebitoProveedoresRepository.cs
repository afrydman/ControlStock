...
{
   public class NotaDebitoProveedoresRepository: DbRepository,INotaRepository
    {
        public NotaDebitoProveedoresRepository(bool local = true)
            : base(local)
        {
        }
        ...
        public bool Insert(NotaData theObject)
        {
            ...
            {
                var sqlEnvio = @"INSERT INTO [dbo].[notaDebitoproveedores]
           ([id]
           ,[idlocal]
           ,[Numero]
           ,[Prefix]
     ...
     VALUES
        ...
        public bool Disable(Guid idObject)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                var sql = @"	update notaDebitoproveedores set Enable = '0' where id  = @id;
                        SELECT @@ROWCOUNT;";
        ...
        public List<NotaData> GetAll()
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                ...
                resultado2.ForEach(x => x.tipo = tipoNota.DebitoProveedor);
        ...
        public NotaData GetByID(Guid idObject)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                ...
                aux.tipo = tipoNota.DebitoProveedor;
                return aux;
            }
        }
        ...
        public NotaData GetLast(Guid idLocal, int Prefix)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                var sql = @"select top 1 " + DEFAULT_SELECT + @"   where nota.Prefix=@Prefix and nota.idlocal=@idlocal  and nota.Numero = (
                            select max(Numero) as 'Numero' from notaDebitoproveedores where Prefix=@Prefix and idlocal=@idLocal ) ";
       ...
       {
           return con.Query<NotaData, LocalData, PersonalData, ProveedorData, CondicionIvaData,NotaData>(sql,
       ...
       public override string DEFAULT_SELECT
       {
           get
           {
               return @"  nota.*, locales.*,Personal.*, proveedores.*,condicioniva.*
                        from notaDebitoproveedores nota
