...
{
   public class NotaDebitoProveedoresDetalleRepository : DbRepository, INotaDetalleRepository
    {
        public NotaDebitoProveedoresDetalleRepository(bool local = true)
            : base(local)
        {
        }
        ...
        public bool InsertDetalle(NotaDetalleData ordenPagoDetalle)
        {
            ...
            {
                var sqlEnvio = @"INSERT INTO [dbo].[notaDebitoProveedores_detalles]
           ([FatherID]
           ,[Description]
           ,[Cantidad]
     ...
     VALUES
        ...
        public List<NotaDetalleData> GetDetalles(Guid idnota)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                var sql = "select * from notaDebitoProveedores_detalles where FatherID = @FatherID";
