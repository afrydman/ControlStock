    ...
    public class IngresoRepository : DbRepository,IIngresoRepository
    {
         ...
         public override string DEFAULT_SELECT
        {
            get { return @" ingresos.*,
                ...
                left join locales on ingresos.idlocal=locales.id
                left join Personal on ingresos.idpersonal= Personal.id
                left join tipoingreso on ingresos.idtiporetiro = tipoingreso.id ";
            }
        }
            ...
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                return
                    con.Query<IngresoData, LocalData, PersonalData, TipoIngresoData, IngresoData>(
                        "select top 1 " + DEFAULT_SELECT + @"  where ingresos.idLocal = @id and ingresos.Prefix = @Prefix order by ingresos.Numero desc  ",(retiro, local,personal , tipo) =>
                {
                    retiro.Local = local;
                    retiro.Personal = personal;
                    retiro.TipoIngreso = tipo;
        ...
        public IEnumerable<IngresoData> OperatorGiveMeData(IDbConnection con, string sql, object parameters)
        {
            throw new NotImplementedException();
        }
        ...
        public List<IngresoData> GetModified(Guid idLocal,int first)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                var sql = "select " + DEFAULT_SELECT + @"  ingresos.modificado = '1' and ingresos.idLocal = @id and ingresos.Prefix=@Prefix";
                IEnumerable<IngresoData> resultado = con.Query<IngresoData, LocalData, PersonalData, TipoIngresoData, IngresoData>(sql, (retiro, local, personal, tipo) =>
                {
                    retiro.Local = local;
                    retiro.Personal = personal;
        ...
        public bool MarkSeen(Guid idLocal,int prefix)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                var sql = @"update ingresos set modificado = '0' where idLocal = @id and Prefix=@Prefix; 
                        SELECT @@ROWCOUNT;";
