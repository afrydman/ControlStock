...
{
    public class TributoNotaDebitoClientesRepository : DbRepository, IGenericChildRepository<TributoNexoData>
    {
        public TributoNotaDebitoClientesRepository(bool local = true)
            : base(local)
        {
        }
        ...
        public bool InsertDetalle(TributoNexoData detalle)
        {
            ...
            {
                var sqlEnvio = @"INSERT INTO [dbo].[notaDebitoClientes_tributo]
                   ([FatherID]
                   ,[idTributo]
        ...
        public List<TributoNexoData> GetDetalles(Guid FatherID)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                ...
                var sql =
                    "select * from notaDebitoClientes_tributo inner join FE_Tributo on notaDebitoClientes_tributo.idTributo = FE_Tributo.id where FatherID = @FatherID";
