...
{
    public class TributoNotaDebitoProveedoresRepository : DbRepository, IGenericChildRepository<TributoNexoData>
    {
        public TributoNotaDebitoProveedoresRepository(bool local = true)
            : base(local)
        {
        }
        ...
        public bool InsertDetalle(TributoNexoData detalle)
        {
            ...
            {
                var sqlEnvio = @"INSERT INTO [dbo].[notaDebitoProveedores_tributo]
                   ([FatherID]
                   ,[idTributo]
        ...
        public List<TributoNexoData> GetDetalles(Guid FatherID)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                ...
                var sql =
                    "select * from notaDebitoProveedores_tributo inner join FE_Tributo on notaDebitoProveedores_tributo.idTributo = FE_Tributo.id where FatherID = @FatherID";
