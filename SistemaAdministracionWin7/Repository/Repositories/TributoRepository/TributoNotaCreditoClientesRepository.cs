...
{
    public class TributoNotaCreditoClientesRepository : DbRepository, IGenericChildRepository<TributoNexoData>
    {
        public TributoNotaCreditoClientesRepository(bool local = true)
            : base(local)
        {
        }
        ...
        public bool InsertDetalle(TributoNexoData detalle)
        {
            ...
            {
                var sqlEnvio = @"INSERT INTO [dbo].[notacreditoClientes_tributo]
                   ([FatherID]
                   ,[idTributo]
        ...
        public List<TributoNexoData> GetDetalles(Guid FatherID)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                ...
                var sql =
                    "select * from notacreditoClientes_tributo inner join FE_Tributo on notacreditoClientes_tributo.idTributo = FE_Tributo.id where FatherID = @FatherID";
