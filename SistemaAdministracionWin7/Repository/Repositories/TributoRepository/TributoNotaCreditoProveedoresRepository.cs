...
{
    public class TributoNotaCreditoProveedoresRepository : DbRepository, IGenericChildRepository<TributoNexoData>
    {
        public TributoNotaCreditoProveedoresRepository(bool local = true)
            : base(local)
        {
        }
        ...
        public bool InsertDetalle(TributoNexoData detalle)
        {
            ...
            {
                var sqlEnvio = @"INSERT INTO [dbo].[notacreditoProveedores_tributo]
                   ([FatherID]
                   ,[idTributo]
        ...
        public List<TributoNexoData> GetDetalles(Guid FatherID)
        {
            using (var con = ConnectionFactory.CreateConnection(_connectionString))
            {
                ...
                var sql =
                    "select * from notacreditoProveedores_tributo inner join FE_Tributo on notacreditoProveedores_tributo.idTributo = FE_Tributo.id where FatherID = @FatherID";
