...
namespace Central
{
    public partial class notaDebitoProveedores : notaBase
    {
        ...
        private void cargarNumero()
        {
            ...
            lblNro.Text = notaDebitoProveedoresService.GetNextNumberAvailable(HelperService.IDLocal, HelperService.Prefix, true);
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (dg == DialogResult.OK)
            {
                ...
                {//(string cmbclase, DateTime fecha, string txtTotal, int Numero, string txtobs, string txtIva, Guid idvendedor, Guid idtercero,DataGridView tabla)
                    NotaData mov = cargoNota(cmbclase.Text, fecha.Value, txtTotal.Text, txtobs.Text, txtIva.Text, ((PersonalData)cmbVendedor.SelectedItem).ID, ((ClienteData)cmbClientes.SelectedItem).ID, tabla, tipoNota.DebitoProveedor, Convert.ToInt32(notaDebitoProveedoresService.GetNextNumberAvailable(HelperService.IDLocal, HelperService.Prefix, false)));

                    bool task = notaDebitoProveedoresService.Insert(mov);
                    if (task)
                    {
